...
def build_user_doc() -> Document:
    ...
    add_callout(doc, "important", "❗ FIRST TURN PAYS FOR THE WHOLE SESSION",
    ...
    add_table(doc,
        ...
        [
            ["First \"Hi\" in a fresh session",
             "≈ 90–150 s",
             "Cold cache. Prefill of system prompt + tool definitions (≈ 23–38K tokens) at ~150 tok/s."],
            ["Follow-up turn (cache hit)",
             "≈ 5–15 s",
             "System prefix served from cache. Only your new message + generation pays compute."],
            ["After switching model mid-session",
             "Back to 1–3 min",
             "Each model has its own cache slot — switching forces a fresh prefill."],
            ["Long session (10+ turns, large files in context)",
             "Grows turn-over-turn",
             "The cached prefix stays cheap; what grows is conversation history. `/compact` keeps it lean."],
        ],
...
def build_admin_doc() -> Document:
    ...
    add_table(doc,
        ...
        [
            ...
            ["Every turn is a cache MISS in the vllm-mlx log",
             "Billing-header strip not applied (shim out of sync), or model alias changed mid-session, or session prefix mutated",
             "tail studio1 log for `System KV cache MISS` lines and compare hashes turn-over-turn. Confirm shim has the `_strip_billing_header` call wired into `/v1/messages`. Restart `ir-claude-router-shim`."],
            ["First turn fast, but follow-up turns slow again",
             "Single-slot cache contention from concurrent traffic on the same backend",
             "Check the vllm-mlx log for STORED events between user turns — a different prefix STOREd means another caller evicted the slot. Pin one model per Mac when possible."],
            ["Tool calls render as a text blob (raw JSON) instead of a `tool_use` block",
             "vllm-mlx tool-call streaming bug for hermes/qwen parsers — JSON streams as content deltas",
            ...
            ["Client gets `\"fetch failed\"` with a Node stack trace exposing the operator's home dir",
             "ccr's default error path used to leak a stack with `/home/<user>/.nvm/.../claude-code-router/dist/cli.js:582:11089`",
             "Already mitigated — the shim's `_sanitized_error_response` returns clean Anthropic-format errors and logs the full upstream body to `data/transient/logs/claude_router_shim.log` for operator debugging."],
        ],
