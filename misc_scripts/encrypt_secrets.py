...
def main():
    ...
    try:
        ...
        # Handle plaintext deletion — always prompt unless --delete-plaintext was
        # passed, in which case skip the extra prompt and delete immediately.
        def _delete_plaintext():
        ...
        if args.delete_plaintext:
            # Flag supplied — confirm once then delete
            response = input(
            ...
            # Interactive prompt even without the flag
            print("Next steps:")
            print()
            ...
            print()
            response = input(
                f"3. Delete the plaintext secrets file for security? (y/N): "
            ).strip().lower()
            if response == 'y':
                _delete_plaintext()
            else:
                print()
                print("   Plaintext file kept. Delete it manually when ready:")
                print(f"   rm {plaintext_path}")
