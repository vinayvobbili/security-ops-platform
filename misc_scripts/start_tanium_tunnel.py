...
TANIUM_ONPREM_IP = os.getenv("TANIUM_ONPREM_IP")
...
def kill_existing():
    ...
    subprocess.run(["pkill", "-f", f"socat.*{LOCAL_SOCAT_PORT}.*{TANIUM_ONPREM_IP}"],
...
def start_tunnel():
    ...
    # Start SSH tunnel
    print(f"  Starting SSH tunnel (sirt-lab-12:{REMOTE_TUNNEL_PORT} -> localhost:{LOCAL_SOCAT_PORT})...")
    ...
    print("=" * 50)
    print(f"\nsirt-lab-12:localhost:{REMOTE_TUNNEL_PORT} --> Tanium On-Prem")
