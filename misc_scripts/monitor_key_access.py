...
Cron example (check every hour):
    0 * * * * cd /path/to/project && python misc_scripts/monitor_key_access.py --alert-webex
"""
...
"""
        ...
        webex_api.messages.create(
            roomId=config.webex_room_id_dev_test_space,
            markdown=message
        )
...
def send_email_alert(_analysis):
...
def main():
    ...
    if state['last_check']:
        last_check = datetime.fromisoformat(str(state['last_check']))
    else:
    ...
    new_state = {
        ...
        'event_count': (state.get('event_count') or 0) + analysis['total']
    }
