...
async function submitAuth(event) {
    ...
    try {
        ...
        const response = await fetch('/api/aide/login', {
            method: 'POST',
            headers: {
        ...
        if (response.ok && data.success) {
            ...
            localStorage.setItem('aideUserEmail', userFullEmail);
...
window.addEventListener('DOMContentLoaded', async () => {
    ...
    if (authModal && authModal.style.display !== 'none') {
        ...
        const storedEmail = localStorage.getItem('aideUserEmail');
        if (storedEmail) {
...
async function submitCreateXTicket(event) {
    ...
    try {
        const response = await fetch('/api/aide/create-x-ticket', {
            method: 'POST',
...
async function submitApprovedTesting(event) {
    ...
    try {
        const response = await fetch('/api/aide/approved-testing', {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
            body: JSON.stringify(data)
        });
...
async function submitIOCHunt(event) {
    ...
    try {
        const response = await fetch('/api/aide/ioc-hunt', {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
            body: JSON.stringify(data)
        });
...
async function submitThreatHunt(event) {
    ...
    try {
        const response = await fetch('/api/aide/threat-hunt', {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
            body: JSON.stringify(data)
        });
...
async function fetchOnCallInfo() {
    ...
    try {
        const response = await fetch('/api/aide/oncall');
        const result = await response.json();

        if (result.success) {
            const info = result.data;
            addBotMessage(`${userName}, the on-call person is ${info.name} - ${info.email_address} - ${info.phone_number}`);
        } else {
            addBotMessage(`Error: ${result.error}`);
        }
