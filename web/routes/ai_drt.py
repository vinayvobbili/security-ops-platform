"""AI DRT page — same-origin reverse-proxy to the standalone AIDRT sidecar (ir-aidrt.service on :8026).
...
Also hosts the vendor deploy portal at /ai-drt/deploy — token-gated zip upload,
...
ai_drt_bp = Blueprint("ai_drt", __name__)

AIDRT_BASE = "http://127.0.0.1:8026"
PROXY_PREFIX = "/ai-drt-app"
...
ACTIVE_DIR = _EXTERNAL / "aidrt"
STAGING_DIR = _EXTERNAL / "aidrt_staging"
BACKUPS_DIR = _EXTERNAL / "aidrt_backups"
AUDIT_LOG = _EXTERNAL / "aidrt_audit.jsonl"

MAX_BACKUPS = 5
MAX_UNCOMPRESSED_BYTES = 200 * 1024 * 1024  # 200 MB — current package is ~13 MB extracted
SERVICE_NAME = "ir-aidrt"
VERSION_ID_RE = re.compile(r"^v\d{8}_\d{6}_[a-f0-9]{6}$")

register_vendor_logs(ai_drt_bp, "ai-drt", SERVICE_NAME, "AI DRT")
...
def _service_status() -> dict:
    try:
        r = requests.get(f"{AIDRT_BASE}/status", timeout=3)
        if r.ok:
...
def _restart_service_and_wait(timeout_s: int = 30) -> dict:
    ...
    while time.time() < deadline:
        try:
            r = requests.get(f"{AIDRT_BASE}/status", timeout=2)
            if r.ok:
...
def _find_package_root(extract_dir: Path) -> Path:
    ...
    if len(children) == 1:
        ...
        if (inner / "main.py").is_file() and (inner / "frontend" / "index.html").is_file():
            return inner
    raise ValueError(
        "Zip is missing the expected AIDRT structure — need main.py and frontend/index.html at the root "
...
@ai_drt_bp.route("/ai-drt")
@log_web_activity
def ai_drt_page():
    return render_template("ai_drt.html")


@ai_drt_bp.route("/ai-drt/deploy")
@log_web_activity
def ai_drt_deploy_page():
    return render_template("ai_drt_deploy.html")


@ai_drt_bp.route("/api/ai-drt/active-version", methods=["GET"])
def ai_drt_active_version():
...
# Sidecar paths that mutate server state — only logged-in users may hit these.
# The lone live example is POST /refresh, which re-runs Azure discovery and
# rewrites the asset registry. Everything else (all reads + the read-only POST
# /chat query) is public so anyone can view the dashboard without logging in.
_PUBLIC_NON_GET_PATHS = {"chat"}


@ai_drt_bp.route(f"{PROXY_PREFIX}/", defaults={"path": ""},
                 methods=["GET", "POST", "PUT", "PATCH", "DELETE"])
@ai_drt_bp.route(f"{PROXY_PREFIX}/<path:path>",
                 methods=["GET", "POST", "PUT", "PATCH", "DELETE"])
def ai_drt_proxy(path):
    # Viewing is open to everyone; editing (any state-changing request) requires
    # a logged-in user. GETs and the read-only /chat query are always allowed.
    if request.method != "GET" and path not in _PUBLIC_NON_GET_PATHS:
    ...
    upstream_url = f"{AIDRT_BASE}/{path}"
...
@ai_drt_bp.route("/api/ai-drt/versions", methods=["GET"])
@login_required
def ai_drt_versions():
    """List active / staged / backup versions + service health."""
...
@ai_drt_bp.route("/api/ai-drt/upload", methods=["POST"])
@require_capability(DEPLOY_SIDECAR, sidecar='ai_drt')
def ai_drt_upload():
    """Accept a zip upload and stage it for later activation."""
...
@ai_drt_bp.route("/api/ai-drt/activate", methods=["POST"])
@require_capability(DEPLOY_SIDECAR, sidecar='ai_drt')
def ai_drt_activate():
...
@ai_drt_bp.route("/api/ai-drt/rollback", methods=["POST"])
@require_capability(DEPLOY_SIDECAR, sidecar='ai_drt')
def ai_drt_rollback():
...
@ai_drt_bp.route("/api/ai-drt/staged/<version_id>", methods=["DELETE"])
@require_capability(DEPLOY_SIDECAR, sidecar='ai_drt')
def ai_drt_delete_staged(version_id):
