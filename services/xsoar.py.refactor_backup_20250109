...
# Configure custom SSL context to handle Zscaler proxy
# This creates a more permissive SSL context that can handle intermediate proxies
...
# In corporate environments with Zscaler, SSL termination causes SSLEOFError with Python 3.13
# We use an environment variable to configure SSL verification behavior
#
...
#
# Configuration:
# - Local dev (macOS): DISABLE_SSL_VERIFY=true (default, auto-detected)
# - VM (Linux): DISABLE_SSL_VERIFY=false (default, auto-detected)
# - Override: Set DISABLE_SSL_VERIFY=true or false explicitly
...
# Note: verify_ssl MUST be False when behind Zscaler proxy (Python 3.13 + SSL EOF error)
    ...
    if hasattr(client, 'api_client') and hasattr(client.api_client, 'rest_client'):
        ...
        if hasattr(rest_client, 'pool_manager'):
            ...
            if not DISABLE_SSL_VERIFY:
                ...
                pool_kwargs['ssl_context'] = _ssl_context  # Use custom SSL context
            else:
