...
def process_tipper_data(threat_tippers):
    ...
    for tipper in threat_tippers:
        ...
        try:
            created_date = datetime.strptime(fields['System.CreatedDate'], '%Y-%m-%dT%H:%M:%S.%fZ')
        except ValueError:
            created_date = datetime.strptime(fields['System.CreatedDate'], '%Y-%m-%dT%H:%M:%SZ')
        ...
        processed_data.append({
            ...
            'Action': action_text,
            'Ticket': fields.get('System.Id', ''),
            'Title': fields.get('System.Title', ''),
            'XSOAR_Link': fields.get('XSOAR_Link', '')
        })
...
def generate_threat_tipper_chart(tippers):
    ...
    if _wm_tag:
        fig.text(0.98, 0.02, _wm_tag, ha='right', va='bottom', fontsize=10,
    ...
    try:
        ...
        for tipper in threat_tippers:
            ...
            if not any(tag in tags for tag in ['Info', 'Low', 'Medium', 'High', 'Critical']):
                print(f"Tipper {tipper.get('id')} missing priority tag. Current tags: {tags}. Setting a default tag of Info")
                fields['System.Tags'] = 'Info'
