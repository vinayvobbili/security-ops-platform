...
def add_reason_text(fig, threatcon_details):
    ...
    if threatcon_color != 'green':
        # Wrap each line individually to preserve bullet point formatting (e.g. "- item")
        wrapped_reason = "\n".join(
            textwrap.fill(line.strip(), width=80, subsequent_indent="  ")
            for line in threatcon_details['reason'].split("\n") if line.strip()
        )
...
def add_fancy_title(ax):
    ...
    # Add branding watermark in top right
    _wm_tag = os.environ.get("WATERMARK_TAG", "")
    if _wm_tag:
        ax.text(1.1, 1.25, _wm_tag,
                ha='right', va='center',
