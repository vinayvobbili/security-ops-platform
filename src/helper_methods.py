...
def log_barnacles_activity(bot_access_token):
    """
    Decorator that logs the alert triage service bot activity.

    Args:
        bot_access_token (str): Access token for bot API

    Returns:
        Decorator for logging the alert triage service bot activity
    """
...
def log_toodles_activity(bot_access_token):
    """
    Decorator that logs the notification service bot activity.
...
def log_web_activity(func):
    ...
    def wrapper(*args, **kwargs):
        if request:
            ...
            if client_ip not in ['<internal-host>', '127.0.0.1', '<internal-host>']:  # don't log the activity for my IP address
                now_eastern = datetime.now(eastern).strftime('%m/%d/%Y %I:%M:%S %p %Z')
