...
from ..models import (
    ...
    DEFAULT_XSIAM_HUNT_HOURS,
)
from .qradar import hunt_qradar
from .crowdstrike import hunt_crowdstrike
from .abnormal import hunt_abnormal
from .xsiam import hunt_xsiam
from .behavioral import (
    run_behavioral_hunt,
    generate_behavioral_hunts,
    validate_logscale_query,
    validate_xql_query,
    DEFAULT_HUNT_PLATFORMS,
)
...
def hunt_iocs(
    ...
    xsiam_hours: int = DEFAULT_XSIAM_HUNT_HOURS,
    ...
    Args:
        ...
        xsiam_hours: Hours to search back in XSIAM (default 30 days)
        tools: List of tools to hunt in (default: qradar + crowdstrike)
               Options: "qradar", "crowdstrike", "abnormal", "xsiam"
               XSIAM is opt-in — not in the default set.
    ...
    logger.info(f"Hunting {total_iocs} IOCs across {tools} (QRadar: {qradar_hours}h, CrowdStrike: {crowdstrike_hours}h, XSIAM: {xsiam_hours}h)...")
    ...
    futures = {}
    with ThreadPoolExecutor(max_workers=4, thread_name_prefix="hunt") as executor:
        if "qradar" in tools:
        ...
        if "xsiam" in tools:
            futures[executor.submit(hunt_xsiam, entities, xsiam_hours)] = ("xsiam", xsiam_hours)
                    ...
                    if result.errors:
                        for err in result.errors:
                            if 'not configured' in err.lower() or 'not available' in err.lower():
                                ...
                                break
                elif tool_name == "xsiam":
                    xsiam_result = result
                    logger.info(f"[hunt] XSIAM complete: {result.total_hits} hits")
                    # Check for XSIAM access issues
                    if result.errors:
                        for err in result.errors:
                            if 'not configured' in err.lower() or 'credentials' in err.lower():
                                access_issues.append("XSIAM API not configured or auth failed")
                                break
    ...
    total_hits = sum(
        r.total_hits for r in [qradar_result, crowdstrike_result, abnormal_result, xsiam_result] if r
    )
    ...
    for tool_result in [qradar_result, crowdstrike_result, abnormal_result, xsiam_result]:
        if not tool_result:
            continue
        ...
        # Collect hostnames from any hit type that carries them
        # (CrowdStrike attaches them to hash hits; XSIAM to every hit type)
        for hit in (tool_result.ip_hits + tool_result.domain_hits + tool_result.url_hits
                    + tool_result.filename_hits + tool_result.hash_hits):
    ...
    return IOCHuntResult(
        ...
        search_hours_qradar=qradar_hours,
        search_hours_crowdstrike=crowdstrike_hours,
        search_hours_xsiam=xsiam_hours,
