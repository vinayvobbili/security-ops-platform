...
LOCK_FILE = "/tmp/sleuth_macbook_sleep_monitor.lock"
...
class MacBookSleepMonitor:
    ...
    def _get_webex_token(self):
        ...
        try:
            ...
            config = get_config()
            return config.webex_bot_access_token_sleuth
    ...
    def _restart_sleuth_bot(self, reason):
        ...
        logger.info(f"🔄 Restarting the security assistant bot bot: {reason}")
        self.last_restart_time = datetime.now()

        try:
            # Use the existing restart script
            restart_script = PROJECT_DIR / "src" / "sleuth" / "restart_sleuth.sh"
            if restart_script.exists():
                ...
                if result.returncode == 0:
                    logger.info("✅ the security assistant bot bot restarted successfully")
                    return True
                else:
        ...
        try:
            ...
            while self.running:
                ...
                if any(keyword in line.lower() for keyword in [
                    ...
                    self._restart_sleuth_bot("System sleep/wake event detected")
                    ...
                    if last_state and last_state != current_state:
                        ...
                        if ("AC Power" in current_state and "Battery Power" in last_state) or \
                           ("Battery Power" in current_state and "AC Power" in last_state):
                            ...
                            self._restart_sleuth_bot("Power source change (likely wake event)")
            ...
            try:
                if self._test_webex_connection():
                    ...
                    if time_since_success > timedelta(minutes=3):
                        ...
                        self._restart_sleuth_bot("Extended WebEx API connectivity loss")
                        ...
                        self._restart_sleuth_bot("Multiple consecutive connectivity failures")
                        consecutive_failures = 0
        ...
        while self.running:
            try:
                ...
                if 6 <= current_hour <= 23:
                    ...
                    self._restart_sleuth_bot("Proactive connection refresh")
