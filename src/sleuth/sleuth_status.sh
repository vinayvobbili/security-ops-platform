...
ps aux | grep "sleuth\.py" | grep -v grep | while read line; do
    echo "  ✅ $line"
done

if ! ps aux | grep "sleuth\.py" | grep -v grep > /dev/null; then
...
echo ""
echo "🚀 Quick Commands:"
echo "  Start the security assistant bot:   ./run_sleuth.sh"
echo "  Kill the security assistant bot:    ./kill_sleuth.sh"
echo "  Restart:         ./restart_sleuth.sh"
