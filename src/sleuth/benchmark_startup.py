...
def run_benchmark(script_path: str, version_name: str, max_wait: int = 120) -> dict:
    ...
    subprocess.run(['./src/sleuth/kill_sleuth.sh'], shell=True, capture_output=True)
    subprocess.run(['./kill_sleuth_optimized.sh'], shell=True, capture_output=True)
    time.sleep(2)
...
def main():
    ...
    test_configs = [
        {
            'script': './src/sleuth/run_sleuth.sh',
