...
def main():
    ...
    schedule.every().monday.at("23:55", eastern).do(qa_tickets.generate, config.webex_room_id_qa_tickets)
    ...
    schedule.every(30).seconds.do(lambda: response_sla_risk_tickets.start(config.webex_room_id_response_sla_risk))
    schedule.every(1).minutes.do(lambda: containment_sla_risk_tickets.start(config.webex_room_id_containment_sla_risk))
