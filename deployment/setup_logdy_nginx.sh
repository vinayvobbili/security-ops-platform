...
echo -n "lab" | sudo htpasswd -i -c /home/vinay/pub/IR/.htpasswd admin
sudo chown vinay:vinay /home/vinay/pub/IR/.htpasswd
sudo chmod 600 /home/vinay/pub/IR/.htpasswd
echo "  ✓ Password configured (username: admin, password: lab)"
echo ""
...
echo "  /toodles      - the notification service bot logs only"
echo "  /msoar        - the case orchestrator bot logs only"
echo "  /money-ball   - MoneyBall bot logs only"
echo "  /jarvais      - Jarvais bot logs only"
echo "  /barnacles    - the alert triage service bot logs only"
