...
if ssh lab-vm "pgrep -f 'python.*webex_bots/aide.py'" > /dev/null; then
    PID=$(ssh lab-vm "pgrep -f 'python.*webex_bots/aide.py'")
...
echo "------------------------------------"
ssh lab-vm "tail -20 ~/security-ops-platform/logs/aide.log" | grep -v "WARNING.*deprecated"

echo ""
echo "🔄 Reconnection Events (last 10):"
echo "----------------------------------"
ssh lab-vm "grep -E '(Triggering.*reconnection|Bot instance cleared|Bot thread|Reconnection requested|up and running)' ~/security-ops-platform/logs/aide.log | tail -15"

echo ""
echo "⚠️  Connection Errors (last 5):"
echo "--------------------------------"
ssh lab-vm "grep -E '(Connection.*error|timed out|Connection aborted|Remote.*closed)' ~/security-ops-platform/logs/aide.log | tail -5"
