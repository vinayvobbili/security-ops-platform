...
SYNTH_SYSTEM_PROMPT = (
    ...
    "request and wrap the key value in that headline in **bold** so it stands out, then lay out any "
    "ADDITIONAL supporting facts as short lines rather than a dense paragraph. Lead each "
    ...
    "for a single-fact answer or a plain negative/inconclusive result. When the headline already "
    "states the answer, that headline IS the whole reply — never restate the same value as a separate "
    "emoji fact line below it (e.g. do NOT follow 'Containment status: **normal**' with a redundant "
    "'🚦 Status: **normal**' line). On each fact line, put the "
