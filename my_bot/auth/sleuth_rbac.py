"""Per-user capability gating for DESTRUCTIVE Sleuth (Webex) tools.

Sleuth runs inside Webex rooms with no per-user authorization: every participant
...
Design choices:
  ...
  * Total visibility. Every destructive attempt — allowed OR denied — is posted to
    a central audit room (WEBEX_ROOM_ID_SLEUTH_AUDIT) via the Sleuth bot, so the
...
# Destructive Sleuth tool name -> required capability token. Tools NOT listed
...
def _is_allowed(user_email: str, capability: str) -> bool:
    ...
    try:
        ...
        logger.error("Sleuth RBAC check failed for %s/%s: %s", email, capability, e)
        return False
...
def guard_tool_call(tool_name: str, tool_args: dict) -> Optional[str]:
    ...
    if not capability:
        if tool_name in _NOTIFY_ONLY_TOOLS:
            ...
            try:
                ...
                logger.error("Sleuth notify audit failed for %s: %s", tool_name, e)
    ...
    try:
        ...
        logger.error("Sleuth RBAC audit failed for %s: %s", tool_name, e)

    if allowed:
        logger.info("Sleuth RBAC: ALLOW %s for %s (cap=%s)", tool_name, user_email or "?", capability)
        return None

    logger.warning("Sleuth RBAC: DENY %s for %s (missing cap=%s)", tool_name, user_email or "?", capability)
...
def _audit_room_id() -> str:
    ...
    return (os.environ.get("WEBEX_ROOM_ID_SLEUTH_AUDIT") or "").strip()


def _bot_token() -> str:
    try:
        from my_config import get_config
        return get_config().webex_bot_access_token_sleuth or ""
    except Exception:
        return ""
...
def _send_audit_card(room: str, token: str, message: str) -> None:
    ...
    try:
        ...
        logger.error("Sleuth audit Webex send failed: %s", e)
...
def _audit(tool_name: str, tool_args: dict, user_email: str, room_id: str,
    ...
    if not room or not token:
        logger.info(
            "Sleuth RBAC audit (no audit room/token configured): %s %s by %s in %s cap=%s",
    ...
    message = (
        f"🛡️ **Sleuth destructive action — {verdict}**\n\n"
        f"- 🧰 **Tool:** `{tool_name}`\n"
...
def _audit_notify(tool_name: str, tool_args: dict, user_email: str, room_id: str) -> None:
    ...
    if not room or not token:
        logger.info(
            "Sleuth notify audit (no audit room/token configured): RAN %s by %s in %s",
            tool_name, user_email or "?", room_id or "?",
        )
        return

    message = (
        f"🔎 **Sleuth live endpoint action — RAN**\n\n"
        f"- 🧰 **Tool:** `{tool_name}`\n"
        f"- 👤 **User:** {user_email or 'unknown'}\n"
        f"- 💬 **Room:** {room_id or 'unknown'}\n"
        f"- 📝 **Args:** {_args_preview(tool_args) or '(none)'}\n"
