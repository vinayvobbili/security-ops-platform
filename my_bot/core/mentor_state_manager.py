...
- Separate ChromaDB path (chroma_mentor) via CodebaseIndexer
...
_mentor_state_manager: Optional["MentorStateManager"] = None


def get_mentor_state_manager() -> "MentorStateManager":
    global _mentor_state_manager
    if _mentor_state_manager is None:
        _mentor_state_manager = MentorStateManager()
    return _mentor_state_manager


class MentorStateManager(SecurityBotStateManager):
    ...
    def __init__(self):
        ...
        # Set MENTOR_M1_ANALYSIS_BASE_URL and MENTOR_M1_ROUTER_BASE_URL in .env.
        import os
        mentor_analysis = os.environ.get("MENTOR_M1_ANALYSIS_BASE_URL")
        mentor_router = os.environ.get("MENTOR_M1_ROUTER_BASE_URL")
        if mentor_analysis:
            self.model_config.m1_analysis_base_url = mentor_analysis
            logger.info(f"the Windows triage agent analysis → {mentor_analysis}")
        if mentor_router:
            self.model_config.m1_router_base_url = mentor_router
            logger.info(f"the Windows triage agent router → {mentor_router}")
...
- Search ONCE with a precise, targeted query — e.g. "CIRT_Contain_Host automation" or "sleuth.py process_incoming_message"
...
- When in doubt between answering directly and using a tool, ALWAYS use the tool"""
    ...
    def _setup_paths(self):
        ...
        self.chroma_documents_path = str(project_root / "chroma_mentor")
