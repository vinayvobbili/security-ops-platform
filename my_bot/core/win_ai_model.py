...
Mirrors my_model.py but uses WinAIStateManager and the Windows triage agent-specific help text.
Session keys are namespaced with 'winai_' to avoid mixing with the security assistant bot sessions.
"""
...
BOT_NAMES = ["the Windows triage agent", "WinAI", "win.ai", "winai", "win_ai"]
...
def win_ai_ask(user_message: str, user_id: str = "default", room_id: str = "default") -> dict:
    """
    the Windows triage agent Q&A — same pattern as ask() in my_model.py but for the tutor bot.

    Session keys are prefixed with 'winai_' to namespace away from the security assistant bot.
    """
    ...
    try:
        ...
        session_key = f"winai_{user_id}_{room_id}"
        ...
        logger.error(f"win_ai_ask failed: {e}")
