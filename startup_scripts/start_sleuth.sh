...
timeout 30 tail -f sleuth.log 2>/dev/null | while read -r line; do
    echo "$line"
...
if pgrep -f "webex_bots/sleuth" > /dev/null; then
    PID=$(pgrep -f 'webex_bots/sleuth')
    echo "✅ the security assistant bot is running (PID: $PID)"
    echo ""
    echo "To view logs: tail -f $PROJECT_DIR/sleuth.log"
else
    echo "❌ Warning: the security assistant bot process not found"
    echo "Check logs: tail -20 $PROJECT_DIR/sleuth.log"
fi
