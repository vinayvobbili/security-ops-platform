...
"$PROJECT_DIR/deployment/restart_log_viewer.sh" 8038 "the threat-intel service Bot" "$PROJECT_DIR/logs/tars.log"
...
# Start new the threat-intel service instance in background
...
echo "Starting the threat-intel service bot..."
echo ""
...
if pgrep -f "webex_bots/tars" > /dev/null; then
    ...
    echo "✅ the threat-intel service is running (PID: $PID)"
    echo ""
    echo "To view logs: tail -f $PROJECT_DIR/logs/tars.log"
else
    echo "❌ Warning: the threat-intel service process not found"
