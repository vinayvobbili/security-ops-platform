...
source "$PROJECT_ROOT/deployment/kill_process.sh"
kill_process_gracefully "webex_bots/jarvis" "the orchestration service" || exit 1
sleep 1

# Restart log viewer to ensure it shows latest logs
"$PROJECT_ROOT/deployment/restart_log_viewer.sh" 8035 "the orchestration service Bot" "$PROJECT_ROOT/logs/jarvis.log"
...
nohup env PYTHONPATH="$PROJECT_ROOT" .venv/bin/python webex_bots/jarvis.py > /dev/null 2>&1 &

echo "Starting the orchestration service bot..."
echo ""
...
if pgrep -f "webex_bots/jarvis" > /dev/null; then
    ...
    echo "✅ the orchestration service is running (PID: $PID)"
    echo ""
    echo "To view logs: tail -f $PROJECT_ROOT/logs/jarvis.log"
else
    echo "❌ Warning: the orchestration service process not found"
