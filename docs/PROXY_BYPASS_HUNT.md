...
FROM events
WHERE LOGSOURCETYPENAME(devicetype) LIKE '%Firewall%'
    AND (SOURCEIP INCIDR '<internal-host>/8' OR SOURCEIP INCIDR '<internal-host>/12' OR SOURCEIP INCIDR '<internal-host>/16')
    AND (DESTINATIONIP INCIDR '<internal-host>/8' OR DESTINATIONIP INCIDR '<internal-host>/12' OR DESTINATIONIP INCIDR '<internal-host>/16')
...
INNER JOIN
    ...
    (SELECT
     ...
     WHERE LOGSOURCETYPENAME(devicetype) LIKE '%Firewall%'
        AND (SOURCEIP INCIDR '<internal-host>/8' OR SOURCEIP INCIDR '<internal-host>/12' OR SOURCEIP INCIDR '<internal-host>/16')
        AND NOT (DESTINATIONIP INCIDR '<internal-host>/8' OR DESTINATIONIP INCIDR '<internal-host>/12' OR DESTINATIONIP INCIDR '<internal-host>/16')
        AND STARTTIME > CURRENT_TIMESTAMP - 24 HOURS
     GROUP BY SOURCEIP
...
2. When the source IP is in any of (<internal-host>/8, <internal-host>/12, <internal-host>/16)
3. When the destination IP is in any of (<internal-host>/8, <internal-host>/12, <internal-host>/16)
