...
This guide explains how to route network traffic from your Mac through the lab-lab VM using an SSH SOCKS5 tunnel.
...
- Creates a new SSH SOCKS5 tunnel to lab-lab
...
```bash
# Proxy for Claude CLI via SSH tunnel to lab-lab
export ALL_PROXY="socks5://localhost:8888"
export all_proxy="socks5://localhost:8888"
```
...
```bash
pkill -f 'ssh.*-D 8888.*lab-lab' && ssh -D 8888 -N -f lab-lab
```
...
ssh lab-lab "echo 'Connection OK'"
...
# and forwards traffic through lab-lab VM

# Kill any existing SSH tunnel on port 8888
pkill -f "ssh.*-D 8888.*lab-lab" 2>/dev/null
...
echo "Chrome launched with SOCKS5 proxy through lab-lab VM"
echo "To close the tunnel: pkill -f 'ssh.*-D 8888.*lab-lab'"
```
...
- The tunnel requires an active SSH connection to lab-lab
