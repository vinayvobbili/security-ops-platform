# the orchestration service Bot Timeout Issue - Root Cause Analysis

**Date**: 2025-11-21
**Investigated By**: Claude Code
**Issue**: the orchestration service bot experiencing frequent 60-second timeout errors on VM

## Executive Summary

The the orchestration service bot (and other bots on the VM) were experiencing **1,338+ timeout errors** due to **connection pool exhaustion**. The default 10-connection pool was insufficient for 6 concurrent bots processing WebSocket messages with thread pools.
...
### 2. Updated the orchestration service Bot Configuration
...
### 1. Applied to the orchestration service (COMPLETED)
```bash
scp src/utils/webex_pool_config.py vinay@lab-lab:/home/vinay/pub/IR/src/utils/
scp webex_bots/jarvis.py vinay@lab-lab:/home/vinay/pub/IR/webex_bots/
```

### 2. Restart the orchestration service Bot
```bash
ssh vinay@lab-lab
