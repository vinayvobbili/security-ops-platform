...
- Gateway: <internal-host> (VMware virtual network)

### Why Messages Are Missed

1. **Firewall Connection Tracking Expiration**:
   - After ~10-20 minutes of idle time, the firewall at `<internal-host>` expires the connection from its state table
...
We're experiencing network connectivity issues on VM inr106 (<internal-host>)
...
1. Are there aggressive connection tracking timeouts on the gateway/firewall at <internal-host>?
...
4. Can we see firewall logs showing connection state table evictions for <internal-host>?
...
REQUESTED ACTION:
1. Review firewall/gateway configuration at <internal-host>
