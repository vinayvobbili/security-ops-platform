# Microsoft Teams the notification service Bot

## Overview

The Teams the notification service bot provides the same functionality as the Webex the notification service bot but operates within Microsoft Teams using the Bot Framework. It provides real-time, websocket-like persistent connections similar to Webex bots.
...
The Teams the notification service bot supports all the same commands as the Webex the notification service bot:
...
```
ERROR: Microsoft Teams the notification service Bot credentials not configured!
```
...
```ini
[Unit]
Description = Teams the notification service Bot
