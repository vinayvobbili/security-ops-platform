#!/usr/bin/env python3
"""
Teams Bot Framework Bot for the notification service with WebSocket Streaming
Provides same functionality as Webex the notification service bot via Bot Framework Streaming
...
3. Set TEAMS_AIDE_APP_ID and TEAMS_AIDE_APP_PASSWORD in .secrets
...
TEAMS_AIDE_APP_ID = CONFIG.teams_aide_app_id or ''  # Application (client) ID from Azure
TEAMS_AIDE_APP_PASSWORD = CONFIG.teams_aide_app_password or ''  # Client secret Value from Azure
TEAMS_AIDE_TENANT_ID = CONFIG.teams_aide_tenant_id or ''  # Directory (tenant) ID from Azure (optional)

# Bot Framework Streaming Settings
SETTINGS = BotFrameworkAdapterSettings(
    app_id=TEAMS_AIDE_APP_ID,
    app_password=TEAMS_AIDE_APP_PASSWORD
)
...
def get_help_message() -> str:
    ...
    return """**Available the notification service Commands:**
...
Use commands like: `@aide ioc 1.2.3.4` or `threat-hunt malware`
"""
...
class TeamsBot(ActivityHandler):
    ...
    async def on_message_activity(self, turn_context: TurnContext) -> None:
        ...
        try:
            ...
            # Execute the notification service command using same business logic
    ...
    async def connect(self) -> None:
        ...
        try:
            ...
            headers = {
                ...
                'User-Agent': 'the notification service-Teams-Bot/1.0'
            }
...
def main() -> None:
    ...
    """
    logger.info("=" * 80)
    logger.info("🚀 Starting the notification service Teams Bot with WebSocket Streaming")
    logger.info("=" * 80)
    ...
    if not TEAMS_AIDE_APP_ID or not TEAMS_AIDE_APP_PASSWORD:
        logger.error("=" * 80)
        logger.error("❌ ERROR: Microsoft Teams the notification service Bot credentials not configured!")
        logger.error("=" * 80)
        logger.error("Please set the following environment variables in your .secrets file:")
        logger.error("  TEAMS_AIDE_APP_ID         - Application (client) ID from Azure")
        logger.error("  TEAMS_AIDE_APP_PASSWORD   - Client secret Value from Azure")
        logger.error("  TEAMS_AIDE_TENANT_ID      - Directory (tenant) ID from Azure (optional)")
        logger.error("=" * 80)
        logger.error("Contact your Azure engineer for these values if you don't have them.")
        logger.error("=" * 80)
        sys.exit(1)

    logger.debug(f"App ID configured: {TEAMS_AIDE_APP_ID[:8]}... (length: {len(TEAMS_AIDE_APP_ID)})")
    logger.debug(f"App Password configured: {'*' * 8}... (length: {len(TEAMS_AIDE_APP_PASSWORD)})")
