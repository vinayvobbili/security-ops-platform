...
# HAL9000 SOC Bot - LLM Agent Architecture (same design as the security assistant bot)
"""
...
logging.getLogger('src.utils.enhanced_websocket_client').setLevel(logging.WARNING)
...
from my_bot.core.mcp_model import (
...
def _format_done_message(response_time: float, metrics: dict = None) -> str:
    ...
    if metrics and metrics.get('total_tokens', 0) > 0 and metrics.get('generation_time', 0) > 0:
        ...
        iter_str = f" | Loops: {metrics['iterations']}" if metrics.get('iterations', 1) > 1 else ""
        tok = (f"{metrics['input_tokens']}→{metrics['output_tokens']}"
               if metrics['input_tokens'] > 0 else str(metrics['output_tokens']))
        return f"{done_prefix} ⚡ Time: **{response_time:.1f}s** ({metrics['prompt_time']:.1f}s eval + {metrics['generation_time']:.1f}s gen) | Tokens: {tok} | TPS: {metrics['tokens_per_sec']:.1f}{ttft_str}{iter_str}{route_str}"
...
class Bot(WebexBot):
    ...
    def process_incoming_message(self, teams_message, activity):
        ...
        try:
            ...
            bot_names = ['HAL9000', 'hal9000', 'Hal9000', 'HAL_9000']
            ...
            # Post-process: convert Markdown tables to Webex-compatible format
            if response_text:
...
def main():
    """HAL9000 main - same architecture as the security assistant bot"""
    bot_name = "HAL9000"
    logger.info("Starting HAL9000 with monolithic LLM agent architecture")
    ...
    print("   tail -f logs/hal9000.log", flush=True)
    print("", flush=True)
