...
logger.warning("=" * 100)
logger.warning(f"🚀 ORACLE BOT STARTED - {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}")
logger.warning("=" * 100)
...
webex_api = configure_webex_api_session(
    WebexTeamsAPI(
        access_token=config.webex_bot_access_token_oracle,
...
class DetectionEngineeringStories(Command):
    ...
    @log_activity(config.webex_bot_access_token_oracle, "oracle_activity_log.csv")
    ...
    def execute(self, message, attachment_actions, activity):
        ...
        try:
            ...
            safe_send_message(
                webex_api,
                room_id,
                markdown=f"📊 **Oracle Bot Status**\n\n{health_status}\n{health_detail}",
                fallback_text=f"Oracle Bot Status: {health_status}"
            )
...
def oracle_bot_factory():
    """Create Oracle bot instance"""
    ...
    cleanup_devices_on_startup(
        config.webex_bot_access_token_oracle,
        bot_name="Oracle"
    )
    ...
    bot = WebexBot(
        ...
        bot_name="Oracle - The Metrics & Analytics Bot",
        threads=True,
...
def oracle_initialization(bot_instance=None):
    """Initialize Oracle commands"""
    if bot_instance:
...
def _shutdown_handler(signum=None, frame=None):
    ...
    logger.warning("=" * 100)
    logger.warning(f"🛑 ORACLE BOT STOPPED - {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}")
    logger.warning("=" * 100)


def main():
    """Oracle main - simplified to use basic WebexBot (keepalive handled by peer_ping_keepalive.py)"""
    ...
    logger.info("Starting Oracle with basic WebexBot")

    # Create bot instance
    bot = oracle_bot_factory()

    # Initialize commands
    oracle_initialization(bot)

    # Run bot (simple and direct)
    logger.info("🚀 Oracle is up and running...")
    print("🚀 Oracle is up and running...", flush=True)
    bot.run()
