...
def orchestrator_bot_factory() -> WebexBot:
    """Create the case orchestrator bot instance"""
...
def orchestrator_initialization(bot_instance: Optional[WebexBot] = None) -> bool:
    """Initialize the case orchestrator commands.
...
def _shutdown_handler(signum=None, frame=None):
    ...
    logger.warning("=" * 100)
    logger.warning(f"🛑 the case orchestrator BOT STOPPED - {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}")
    logger.warning("=" * 100)


def main():
    """the case orchestrator main - uses resilience framework for automatic reconnection and firewall handling"""
    ...
    logger.warning("=" * 100)
    logger.warning(f"🚀 the case orchestrator BOT STARTED - {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}")
    logger.warning("=" * 100)
    ...
    # Create bot instance
    bot = orchestrator_bot_factory()

    # Initialize commands
    orchestrator_initialization(bot)

    # Run bot (simple and direct)
    logger.info("🚀 the case orchestrator is up and running...")
    print("🚀 the case orchestrator is up and running...", flush=True)
    bot.run()
