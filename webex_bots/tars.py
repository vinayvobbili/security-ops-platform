...
class GetBotHealth(Command):
    ...
    def execute(self, message, attachment_actions, activity):
        ...
        status_card = AdaptiveCard(
            body=[
                TextBlock(
                    text="🤖 the threat-intel service Bot Status",
...
def tars_bot_factory():
    ...
    approved_bot_emails = [
        ...
        CONFIG.webex_bot_email_jarvis,  # the orchestration service bot
    ]
    ...
    return WebexBot(
        ...
        bot_name="the threat-intel service - The Tanium Assistant",
        threads=True,
        log_level="ERROR",
...
def tars_initialization_with_tracking(bot_instance, resilient_runner):
    """Initialize the threat-intel service with message activity tracking for idle detection"""
    ...
    return tars_initialization(bot_instance)


def main():
    """the threat-intel service main with resilience framework"""
    ...
    resilient_runner = ResilientBot(
        bot_name="the threat-intel service",
        bot_factory=tars_bot_factory,
        initialization_func=lambda bot: tars_initialization_with_tracking(bot, resilient_runner),
        max_retries=5,
