...
def get_url_card():
    ...
    """
    try:
        lab_urls = list_handler.get_list_data_by_name('METCIRT URLs')
        actions = []
...
class ContainmentStatusCS(Command):
    ...
    def execute(self, message, attachment_actions, activity):
        ...
        host_name_cs = host_name_cs.replace("METCIRT_the notification service status", "").strip()
...
def main():
    """the notification service main - supports three modes of operation"""
    if CONFIG.should_use_proxy_resilience:
        # Full resilience: ZScaler features + auto-reconnect
        logger.info("Starting the notification service with FULL resilience (SSL config, WebSocket patching, device cleanup, auto-restart)")
        ...
        logger.info("Starting the notification service with LITE resilience (auto-reconnect only, no ZScaler features)")
        from src.utils.bot_resilience import ResilientBot
        ...
        logger.info("Starting the notification service with STANDARD mode (no auto-reconnect, no resilience features)")
