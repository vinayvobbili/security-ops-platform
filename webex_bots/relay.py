...
logger.warning("=" * 100)
logger.warning(f"🚀 RELAY BOT STARTED - {dt_for_marker.now().strftime('%Y-%m-%d %H:%M:%S')}")
logger.warning("=" * 100)
...
config = get_config()
bot_token = config.webex_bot_access_token_relay
...
try:
    ...
    company_logo = ""  # logo asset not bundled in this build
...
class BotStatusCommand(Command):
    ...
    @log_activity(config.webex_bot_access_token_relay, "relay_activity_log.csv")
...
class AnnounceThreatcon(Command):
    ...
    def execute(self, message, attachment_actions, activity):
        try:
            ...
            WebexTeamsAPI(access_token=config.webex_bot_access_token_aide).messages.create(
...
def get_random_relay_quote():
    """Get a random nautical quote."""
    return random.choice(RELAY_QUOTES)
...
class AlertTriageBot(WebexBot):
    ...
    def __init__(self, *args, **kwargs):
        ...
        # Claude API client — uses relay_claude_api_key (RELAY_CLAUDE_API_KEY)
        # so that Pokédex never picks up this key and stays on the local LLM.
        self._claude = anthropic.Anthropic(api_key=cfg.relay_claude_api_key) if cfg.relay_claude_api_key else None
        ...
        self._cost_file = ROOT_DIRECTORY / "data" / "transient" / "relay_cost.json"
    ...
    def _ask_claude(self, user_id: str, text: str,
        ...
        empty_stats = {}
        if not self._claude:
            return "❌ AI not available — RELAY_CLAUDE_API_KEY not configured.", empty_stats
    ...
    def process_incoming_message(self, teams_message, activity):
        ...
        text = re.sub(r'(?i)^(dnr[_\s]*)?relay[,\s]*', '', text).strip()
        if not text:
        ...
        try:
            thinking_msg = webex_api.messages.create(
                roomId=teams_message.roomId,
                parentId=parent_id,
                text=random.choice(RELAY_QUOTES),
            )
            ...
            def update_thinking_message():
                ...
                while thinking_active.is_set() and counter <= max_edits:
                    ...
                    if thinking_active.is_set():
                        try:
                            new_message = random.choice(RELAY_QUOTES)
                            resp = _requests.put(
        ...
        try:
            ...
            if stats:
                log_llm_usage(
                    bot="relay", actor=actor_name, prompt_preview=text,
            ...
            logger.warning(f"Failed to log relay conversation: {e}")
...
def relay_bot_factory():
    ...
    configured_users = config.relay_approved_users.split(',')
    bot_emails = [
        config.webex_bot_email_aide,
...
def relay_initialization(bot_instance=None):
...
def _shutdown_handler(signum=None, frame=None):
    ...
    logger.warning("=" * 100)
    logger.warning(f"🛑 RELAY BOT STOPPED - {dt_for_marker.now().strftime('%Y-%m-%d %H:%M:%S')}")
    logger.warning("=" * 100)
...
def main():
    ...
    # Create bot instance
    bot = relay_bot_factory()

    # Initialize commands
    relay_initialization(bot)
