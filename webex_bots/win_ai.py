...
Architecture: mirrors pokedex.py but uses WinAIStateManager and win_ai_ask().
"""
...
from my_bot.core.win_ai_model import initialize_win_ai, win_ai_ask
from my_bot.core.win_ai_state_manager import get_win_ai_state_manager
from src.utils.bot_messages import WIN_AI_THINKING_MESSAGES, DONE_MESSAGES
...
logger.warning("=" * 100)
logger.warning(f"WIN.AI BOT STARTED - {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}")
logger.warning("=" * 100)

CONFIG = get_config(bot_name='winai')
WEBEX_ACCESS_TOKEN = CONFIG.webex_bot_access_token_winai
WEBEX_BOT_EMAIL = CONFIG.webex_bot_email_winai
eastern = timezone('US/Eastern')
LOG_FILE_DIR = PROJECT_ROOT / 'data' / 'transient' / 'logs'

if not WEBEX_ACCESS_TOKEN:
    logger.error("WEBEX_BOT_ACCESS_TOKEN_WINAI is required")
    sys.exit(1)
...
def initialize_bot() -> bool:
    ...
    try:
        ...
        state_manager = get_win_ai_state_manager()
...
class WinAIBot(WebexBot):
    ...
    def process_incoming_message(self, teams_message, activity):
        ...
        from my_bot.core.win_ai_model import BOT_NAMES
        ...
        from my_bot.core.win_ai_model import _is_clear_session
        if _is_clear_session(cleaned):
            from my_bot.core.session_manager import get_session_manager
            session_key = f"winai_{teams_message.personEmail}_{teams_message.roomId}"
        ...
        try:
            thinking_msg = self.teams.messages.create(
                ...
                parentId=parent_id,
                text=random.choice(WIN_AI_THINKING_MESSAGES),
            )
            thinking_active.set()
                ...
                while thinking_active.is_set() and counter <= 9:
                    ...
                    if thinking_active.is_set():
                        _edit_message(
                            ...
                            thinking_msg.id,
                            f"{random.choice(WIN_AI_THINKING_MESSAGES)} ({counter * 30}s)",
                        )
                        counter += 1
        ...
        try:
            ...
            logger.error(f"win_ai_ask error: {e}")
...
def _shutdown_handler(_signum=None, _frame=None):
    logger.warning("=" * 100)
    logger.warning(f"WIN.AI BOT STOPPED - {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}")
    logger.warning("=" * 100)
...
def main():
    ...
    print(f"  Logs: tail -f {PROJECT_ROOT}/logs/win_ai.log", flush=True)
